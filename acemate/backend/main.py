...
app = FastAPI(
    title="AceMate API",
    description="Backend API for the AceMate AI Learning Assistant",
    version="1.0.0",
)
...
# CORS
# ---------------------------------------------------------
# Allow both localhost and 127.0.0.1 because Vite can use either.
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173",
...
def chat(request: ChatRequest):
    ...
    return {
        "reply": (
            "Sorry, AceMate is temporarily unable to generate a response. "
            "Please check the Gemini API key or Ollama setup."
        ),
        "model": None,
        "status": "error",
    }
